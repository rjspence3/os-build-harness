...
- **Phase 0, 1, 2, 3, 5 = DONE.** Remaining: Phase 4 (Tier-2/3 breadth), Phase 6 (hardening/flywheel). One Phase-5 nicety left: a fully-unattended `RUN_MODE=headless` run (vs the tight-poll dispatch used here) — the drive loop + machine-done are both proven; that's just the launch vehicle.
