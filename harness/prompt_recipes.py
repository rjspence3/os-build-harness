...
# ── M1 Wave 1: ODC-native recipes surfaced by RECIPE_GAPS (B2-1..B2-5) ───────
def rest_consume(params: dict) -> str:
    """B2-1: author a CONSUMED REST API integration (ODC consumes REST natively). params: name, base_url,
    methods:[{name, http, path, request_struct?, response_struct?, is_list?}], auth?(None|ApiKey|OAuth note).
    A consumed integration is CALLED from server logic (distinct from an EXPOSED endpoint). The base URL is
    per-stage overridable — set it on the integration, never hardcoded per method."""
    name = _p(params, "name", required=True)
    base = _p(params, "base_url", required=True)
    methods = _p(params, "methods", [], required=True)
    auth = _p(params, "auth", "None")

    def _m(m: dict) -> str:
        req = f", request body -> Structure {m['request_struct']}" if m.get("request_struct") else ""
        rsp = (f", response -> {'List of ' if m.get('is_list') else ''}Structure {m['response_struct']}"
               if m.get("response_struct") else "")
        return f"   - a {m.get('http', 'GET')} method '{m['name']}' at path {m.get('path', '/')}{req}{rsp}"

    return (f"{_PREAMBLE}\n\n"
            f"Author a CONSUMED REST API integration named {name} so this app can CALL an external service (a "
            f"consume integration — Logic > Integrations > REST — NOT an exposed endpoint). Set the integration's "
            f"base URL to {base} (ODC makes it a per-stage-overridable configuration — set it ONCE on the "
            f"integration, do NOT hardcode it inside each method). Authentication: {auth}. Add these methods:\n"
            f"{chr(10).join(_m(m) for m in methods)}\n"
            f"For each method, the RESPONSE structure is AUTO-GENERATED by the integration from the response JSON — do "
            f"NOT pre-create or map an app-level Structure for the response (the integration owns its own response "
            f"type; forcing an existing app Structure just leaves it unused). If a method sends a REQUEST body, author "
            f"a Structure for that request payload. An array response becomes a List of the integration's generated "
            f"response structure. The methods become server-callable actions under {name}. After authoring, run model "
            f"validation and report errors. Do not publish.")


def scheduled_timer(params: dict) -> str:
    """B2-2: a Timer on a RECURRENT schedule (ODC Timers do daily/weekly/interval — not just WhenPublished,
    which seed-entity hardcodes). params: name, action(the server action it runs), schedule(whenPublished|
    daily|weekly|interval), at?(time-of-day for daily/weekly). The action MUST be idempotent: ODC auto-retries
    a Timer that exceeds its timeout, and caps a run at 60 min (keep work < ~30 min; chunk larger batches)."""
    name = _p(params, "name", required=True)
    action = _p(params, "action", required=True)
    schedule = _p(params, "schedule", "daily")
    at = _p(params, "at")
    when = {"whenpublished": "run once WhenPublished",
            "daily": f"recur DAILY{' at ' + at if at else ''}",
            "weekly": f"recur WEEKLY{' at ' + at if at else ''}",
            "interval": "recur on a fixed INTERVAL"}.get(schedule.lower(), f"recur {schedule}")
    return (f"{_PREAMBLE}\n\n"
            f"Author a Timer named {name} whose Action is the EXISTING server action {action} and whose schedule is "
            f"to {when}. Use the Timer's recurrent-schedule properties (NOT only WhenPublished — an ODC Timer supports "
            f"a daily/weekly/interval recurrence). {action} MUST be IDEMPOTENT: ODC auto-retries a Timer that exceeds "
            f"its timeout, so a re-run must not double-process (guard on a marker/already-handled filter). A Timer run "
            f"is capped at 60 minutes — keep each run under ~30 min; if the batch is larger, process a bounded chunk "
            f"per run and let the next scheduled run continue. After authoring, run model validation. Do not publish.")
...
def conditional(params: dict) -> str:
    """B2-4: gate a widget's visibility on a boolean expression (native conditional render). params: screen,
    component_id, visible_when(the OutSystems expression). Authored as the widget's Visible expression — client-
    reactive, re-evaluated when its inputs change. Do NOT delete+recreate the widget (property-only mutation is
    flaky, but Visible IS a client-reactive IExpression that can be set post-creation via SetVisible)."""
    screen = _p(params, "screen", required=True)
...
def derived_field(params: dict) -> str:
    """B2-5: render a COMPUTED value via an Expression widget — a Text widget is literal-only, so a calculated
    field (DTI, net worth, monthly payment) needs an Expression whose Value is the formula. params: screen,
    component_id, formula(the OutSystems expression), label?."""
    screen = _p(params, "screen", required=True)
    comp = _p(params, "component_id", required=True)
    formula = _p(params, "formula", required=True)
    label = _p(params, "label", "")
    label_txt = f" Precede it with a Text label \"{label}\"." if label else ""
    return (f"{_PREAMBLE}\n\n"
            f"On the {screen} screen, render the COMPUTED value for \"{comp}\" using an EXPRESSION widget (NOT a Text "
            f"widget — a Text widget's Text is a literal and cannot hold a formula). Create an Expression widget, set "
            f"its data-spec-id attribute to \"{comp}\", and SetValue to the expression: {formula}.{label_txt} The "
            f"value recomputes reactively when its inputs change. After authoring, run model validation. Do not publish.")
...
RECIPES = {
    ...
    # M1 Wave 1 (RECIPE_GAPS B2-1..B2-5)
    "rest-consume": rest_consume,
    "scheduled-timer": scheduled_timer,
    "excel-import": excel_import,
    "conditional": conditional,
    "derived-field": derived_field,
}
...
def plan_from_spec(spec: dict, *, kpi_model_api_fallback: bool = False) -> list[dict]:
    ...
    # B1 (RECIPE_GAPS) fail-closed: ODC attributes can't be list-valued — refuse to plan an
    # unbuildable data model (harness-verify --phase spec gives the same, richer, message first).
    for _e in all_entities:
        for _a in _e.get("attributes", []) or []:
            _dt = _a.get("dataType")
            if isinstance(_dt, str) and (_dt.rstrip().endswith("[]") or _dt.strip().lower().startswith(("list", "array"))):
                raise ValueError(
                    f"array-valued attribute {_e.get('name', '?')}.{_a.get('name', '?')} "
                    f"(dataType '{_dt}') is not representable in ODC — model it as a child entity "
                    f"+ FK (or a CSV Text column). See RECIPE_GAPS.md B1.")
    ...
    # M1 Wave 1: CONSUMED REST integrations (B2-1) — request/response Structures already emitted above.
    for integ in spec.get("integrations", []) or []:
        if integ.get("kind") == "RestApi" and integ.get("direction", "consume") == "consume" and integ.get("methods"):
            steps.append({"recipe": "rest-consume", "why": f"consume REST {integ['name']}",
                          "params": {"name": integ["name"], "base_url": integ.get("baseUrl", ""),
                                     "methods": integ["methods"], "auth": integ.get("auth", "None")}})
    if screens:
        scaffold = []
    ...
    for s in screens:
        ...
        # M1 Wave 1: per-component conditional render (B2-4) + derived/computed value (B2-5).
        for c in s.get("components", []):
            if c.get("visibleWhen"):
                steps.append({"recipe": "conditional",
                              "why": f"{s['id']}.{c['id']} visibleWhen",
                              "params": {"screen": s["id"], "component_id": c["id"],
                                         "visible_when": c["visibleWhen"]}})
            if c.get("computed"):
                steps.append({"recipe": "derived-field",
                              "why": f"{s['id']}.{c['id']} computed",
                              "params": {"screen": s["id"], "component_id": c["id"],
                                         "formula": c["computed"], "label": c.get("label", "")}})
    ...
    # M1 Wave 1: scheduled/recurrent Timers (B2-2) — after logic, since a Timer's Action must already exist.
    for tm in spec.get("timers", []) or []:
        steps.append({"recipe": "scheduled-timer", "why": f"timer {tm['name']} ({tm.get('schedule', 'daily')})",
                      "params": {"name": tm["name"], "action": tm["action"],
                                 "schedule": tm.get("schedule", "daily"), "at": tm.get("at")}})
