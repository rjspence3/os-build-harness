"""Log into Portal6 (HomeBankingPortal6) and screenshot the Dashboard via CDP Chrome.

Portal6 variant of cdp_login_screenshot.py (URLs + page-match retargeted to
HomeBankingPortal6). Same metrics + assertion logic. Reads creds from .env
(HB_PORTAL_USER / HB_PORTAL_PASS) — never hardcoded.

Usage:
  .venv/bin/python scripts/cdp_login_screenshot_p6.py [out.png] [--assert "k=v,k<=v"]
...
OUT = _args[0] if _args else "compare/portal6_dashboard.png"
    ...
    try:
        ...
        if not skip_login and "Login" in page.url:
            ...
            if not (pwd and usr):
                ...
                page.screenshot(path="compare/portal6_login_debug.png")
                return 3
            usr.fill(user)
            pwd.fill(pw)
            pwd.press("Enter")
        ...
        if "Login" in page.url:
            ...
            page.screenshot(path="compare/portal6_login_failed.png")
            return 4
